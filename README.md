...
<head>
  ...
  <title>אקדמיית אלעד רבי — לידים</title>
...
<body class="min-h-screen p-6">
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-2xl font-bold text-white">✂️ אקדמיית אלעד רבי</h1>
        <p class="text-sm text-gray-500 mt-1">דשבורד לידים — עדכון אחרון: <span id="last-updated">—</span></p>
      </div>
      <button id="refresh-btn" onclick="loadLeads()">⟳ רענן</button>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8" id="stats-row">
      <div class="stat-card rounded-xl p-4 text-center">
        ...
        <div class="text-xs text-gray-500 mt-1">סה"כ לידים</div>
      </div>
      <div class="stat-card rounded-xl p-4 text-center">
        <div class="text-3xl font-bold text-green-400" id="stat-hot">—</div>
        <div class="text-xs text-gray-500 mt-1">🔥 חם</div>
      </div>
      <div class="stat-card rounded-xl p-4 text-center">
        <div class="text-3xl font-bold text-yellow-400" id="stat-avg">—</div>
        <div class="text-xs text-gray-500 mt-1">⭐ ממוצע</div>
      </div>
      <div class="stat-card rounded-xl p-4 text-center">
        <div class="text-3xl font-bold text-zinc-400" id="stat-weak">—</div>
        <div class="text-xs text-gray-500 mt-1">❄️ חלש</div>
      </div>
      <div class="stat-card rounded-xl p-4 text-center">
        <div class="text-3xl font-bold text-violet-400" id="stat-exp">—</div>
        <div class="text-xs text-gray-500 mt-1">✂️ מנוסה</div>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-5">
      ...
      <button class="filter-btn" onclick="setFilter('hot', this)">🔥 חם</button>
      <button class="filter-btn" onclick="setFilter('average', this)">⭐ ממוצע</button>
      <button class="filter-btn" onclick="setFilter('weak', this)">❄️ חלש</button>
      <button class="filter-btn" onclick="setFilter('disqualified', this)">🚫 פסול</button>
      <button class="filter-btn" onclick="setFilter('experienced', this)">✂️ מנוסה</button>
    </div>
    ...
    <p class="text-xs text-gray-600 mt-4 text-center">מתרענן אוטומטית כל 60 שניות</p>
  </div>
  ...
  <script>
    ...
    const categoryBadge = cat => {
      if (!cat) return `<span class="badge-weak stage-pill">בתהליך</span>`;
      const map = {
        hot:          `<span class="badge-hot rounded-full px-3 py-0.5 text-xs font-semibold">🔥 חם</span>`,
        average:      `<span class="badge-avg rounded-full px-3 py-0.5 text-xs font-semibold">⭐ ממוצע</span>`,
        weak:         `<span class="badge-weak rounded-full px-3 py-0.5 text-xs font-semibold">❄️ חלש</span>`,
        disqualified: `<span class="badge-disq rounded-full px-3 py-0.5 text-xs font-semibold">🚫 פסול</span>`,
      };
    ...
    const fmtPhone = p => p ? p.replace('+972', '0') : '—';

    const preferredCallLabel = v => ({ zoom:'זום 💪', phone:'טלפון 📞' })[v] || v || '—';
    const hoursLabel = v => ({ morning:'בוקר', afternoon:'צהריים', evening:'ערב' })[v] || v || '—';
    ...
    async function loadLeads() {
      try {
        ...
        document.getElementById('table-container').innerHTML =
          `<div class="loading text-red-400">שגיאה בטעינת הנתונים: ${e.message}</div>`;
      }
    }
    ...
    function renderTable() {
      ...
      if (!leads.length) {
        document.getElementById('table-container').innerHTML =
          `<div class="loading">אין לידים להצגה</div>`;
        return;
      }
      ...
      const rows = leads.map(l => `
        <tr>
          ...
          <td class="text-center">
            ...
            }">${l.score !== null ? Number(l.score).toFixed(1) : '—'}</span>
          </td>
          <td>${categoryBadge(l.lead_category)}
            ${l.is_experienced ? ' <span class="badge-exp rounded-full px-2 py-0.5 text-xs">✂️</span>' : ''}
          </td>
          <td class="text-center">
        ...
        <table>
          <thead>
            <tr>
              <th>שם</th>
              <th>טלפון</th>
              <th class="text-center">ניקוד</th>
              <th>קטגוריה</th>
              <th class="text-center">שלב</th>
              <th>סוג שיחה</th>
              <th>זמינות</th>
              <th>נוצר</th>
            </tr>
